...
export const generate = async (): Promise<void> => {
    ...
    for (const configPath of _config.run) {
        ...
        if (contributingMd) writeFile(`${process.env.NODE_ENV === 'development' ? 'out' : `${configPath}`}`, contributingMd, "CONTRIBUTING.md");
        if (codeOfConductMd) writeFile(`${process.env.NODE_ENV === 'development' ? 'out' : `${configPath}`}`, codeOfConductMd, "CODE_OF_CONDUCT.md");
